      ...
      <div class="form-row">
        ...
        <div class="form-group col-md-4">
          ...
          <select name="status" class="form-control">
            <?php $status_list = ['aktif'=>'Aktif','mutasi_keluar'=>'Mutasi Keluar','mutasi_masuk'=>'Mutasi Masuk','lulus'=>'Lulus','keluar'=>'Keluar']; ?>
            <?php foreach($status_list as $val => $label): ?>
              <option value="<?= $val ?>" <?= ($val == $siswa->status ? 'selected' : '') ?>><?= $label ?></option>
            <?php endforeach; ?>
          </select>
        </div>
      </div>
